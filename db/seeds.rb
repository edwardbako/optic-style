...
if Notification.count == 0
  ...
  Notification.create(
    icon: 0,
    text: """
      Продукты в каталоге можно структурировать с помощью <b>папок</b>. Новый продукт или новая папка создаются в
      текущем разделе. Если вы хотите, чтобы продукт показывался в каталоге на сайте, <b>опубликуйте</b> его,
      нажав Опубликовать справа.
    """
  )
  Notification.create(
    icon: 1,
    text: "<b>Внимание!</b> При удалении папки, все содержащие в ней продукты и папки также удалятся."
  )
  Notification.create(
    icon: 2,
    text: """
      Статья состоит из нескольких разделов. По умолчанию добавлены заставка и текстовый блок. Внизу есть кнопки для
      добавления других разделов. <b>Вид статьи не является окончательным!</b> После сохранения, нажмите на
      предпросмотр. Так будет выглядеть статья после публикации.
    """
  )
